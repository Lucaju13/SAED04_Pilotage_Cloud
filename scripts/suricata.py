...
class FileDownloader:
    ...
    def download_files(self, file_prefix, num_files):
        for i in range(1, num_files + 1):
            file_name = f"{file_prefix}.json.{i}.gz"
    ...
    def extract_files(self, file_prefix, num_files):
        for i in range(1, num_files + 1):
            input_file = os.path.join(self.output_directory, f"{file_prefix}.json.{i}.gz")
            output_file = os.path.join(self.output_directory, f"{file_prefix}.json.{i}")
...
base_url = "https://registry.iutbeziers.fr:4443/public.php/webdav/suricata/log"
